    ...
    <div className="p-6 space-y-6 bg-orange-50 min-h-full">
      ...
      <div className="flex items-center justify-between bg-white rounded-xl p-6 shadow-sm border border-orange-200">
        <div>
          ...
          <p className="text-orange-700 mt-1">Gerencie sua base de clientes e histórico de atendimentos</p>
        </div>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-10 border-2 border-orange-200 focus:border-orange-400 bg-white text-orange-900 rounded-lg h-12"
            />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-white shadow-sm border border-orange-200 rounded-xl">
        <CardHeader>
          <CardTitle className="text-orange-900">Lista de Clientes ({clientesFiltrados.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {clientesFiltrados.map((cliente) => (
              <div key={cliente.id} className="border border-orange-100 rounded-lg p-4 hover:bg-orange-50 transition-colors">
                <div className="flex flex-col md:flex-row md:items-center justify-between space-y-3 md:space-y-0">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-700 text-white flex items-center justify-center font-semibold">
                        {cliente.nome.split(' ').map(n => n[0]).join('').substring(0, 2)}
                      </div>
                      <div>
                        <h3 className="font-semibold text-lg text-orange-900">{cliente.nome}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        <p className="text-orange-600">Último Atendimento</p>
                        <p className="font-medium flex items-center text-orange-900">
                          <Calendar className="h-3 w-3 mr-1 text-orange-700" />
                          {cliente.ultimoAtendimento}
                        </p>
                      </div>
                      <div>
                        <p className="text-orange-600">Total de Atendimentos</p>
                        <p className="font-medium text-orange-900">{cliente.totalAtendimentos} atendimentos</p>
                      </div>
                    </div>
                  </div>
                </div>
